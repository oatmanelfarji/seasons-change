...
import {
	getAstronomicalCalendar,
	getHouseForDate,
} from "./astronomical-utils.ts";
...
try {
	...
	if (house7Leap) {
		...
		console.log(
			`Dates: ${format(house7Leap.calculatedStartDate, "yyyy-MM-dd")} to ${format(house7Leap.calculatedEndDate, "yyyy-MM-dd")}`,
		);
	...
	if (house7Standard) {
		...
		console.log(
			`Dates: ${format(house7Standard.calculatedStartDate, "yyyy-MM-dd")} to ${format(house7Standard.calculatedEndDate, "yyyy-MM-dd")}`,
		);
	...
	console.log(
		`Date: 2028-02-29 falls in: ${houseOnLeapDay?.englishName} (House ${houseOnLeapDay?.id})`,
	);
} catch (e) {
